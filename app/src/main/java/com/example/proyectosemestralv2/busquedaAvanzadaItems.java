                            ...
                            public void onDataChange(DataSnapshot dataSnapshot) {
                                if (dataSnapshot != null) {
                                    for(int z = 0; z<dataSnapshot.getChildrenCount();z++){
                                        String contBA = String.valueOf(z);
                                        String result = String.valueOf(dataSnapshot.child(contBA).child("codigo_correlativo").getValue(long.class));
                                        if(!resultados.contains(result)){resultados.add(result);}
                                    }
                                }
                            }
                            @Override
